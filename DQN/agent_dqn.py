...
# wandb.init(project='sweep_test',entity='avarude')
"""
...
np.random.seed(595)
random.seed(595)
#Params from the slides provided on the class github page
EPISODES = 200000
...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        #defing two weights as Q and Q_cap one for training a constant target
    ...
    # def epsilon_greedy(self,best_action,epsilon):
    #     action=np.ones(self.action_count)*epsilon/self.action_count
    #     action[best_action]+= 1-epsilon
    #     return np.random.choice(np.arange(self.action_count), p=action)
        ...
        while np.mean(reward_buffer) < 50:
            ...
            while True:
                if episode > DECAY_EPSILON_AFTER:
                    epsilon = max(EPSILON_END,epsilon-(epsilon-EPSILON_END)/EPISODES)
                else:
                    epsilon = EPSILON
                ...
                # action=self.epsilon_greedy(self.make_action(curr_state),epsilon)
            ...
            # writer.add_scalar("Epsilon vs Time_stamp", epsilon, t_stamp)
            if episode % 100 == 0:
                print("Mean reward(100) vs episode", np.mean(reward_buffer),episode)
                # wandb.log({"reward":np.mean(reward_buffer),"episode":episode,"epsilon":epsilon,"timestamp":t_stamp})
                # writer.add_scalar("Mean reward(100) vs Episode", np.mean(reward_buffer), episode)
            if episode % TARGET_UPDATE_FREQUENCY == 0:
                self.Q_cap.load_state_dict(self.Q.state_dict())
            if episode % SAVE_MODEL_AFTER == 0:
                # if exp_reward <= np.mean(reward_buffer):
                torch.save(self.Q.state_dict(), "simple_dqn_model.pth")
    ...
    def optimize(self):
        ...
        # state_action_values=sav.gather(1, action_batch) # this fucntion was giving error so i manually implemented what this fucntion was supposed to do
        ...
        # Compute Huber loss
        #check the input dimensions for the loss function
